...
<g id="Camada-1" serif:id="Camada 1" fill-rule="nonzero">
  ...
  <defs>
	...
	function __construct( $plugin ) {
		...
		add_action( 'pre_post_update', [ $this, 'maybe_clear_cache_on_post_update' ], 10, 2 );
	}
	 ...
	 */
	function maybe_clear_cache_on_post_update( $post_id, $data ) {
		...
		if ( $settings->cache_clear_home_on_post_edit ) {
			...
			$blog_page_url = $this->plugin->ezcache->get_relative_url( get_post_type_archive_link( 'post' ) );
			$this->plugin->ezcache->clear_cache_url( $blog_page_url );
		}
	}
	...
	protected function getJsTraslations() {
		return [
			...
			'performance'            => __( 'Performance', 'ezcache' ),

			// Performance page translations
			'cache_preload'          => __( 'Cache Preload', 'ezcache' ),
			'status'                 => __( 'Status', 'ezcache' ),
			'processed'              => __( 'Processed', 'ezcache' ),
			'remaining'              => __( 'remaining', 'ezcache' ),
			'run_preload_now'        => __( 'Run Preload Now', 'ezcache' ),
			'stop_preload'           => __( 'Stop Preload', 'ezcache' ),
			'preload_completed'      => __( 'Completed', 'ezcache' ),
			'preload_last_run'       => __( 'Last preload', 'ezcache' ),
			'pages'                  => __( 'pages', 'ezcache' ),
			'time_ago'               => __( 'ago', 'ezcache' ),
			'time_just_now'          => __( 'just now', 'ezcache' ),
			'preload_settings'       => __( 'Preload Settings', 'ezcache' ),
			'enable_preload'         => __( 'Enable Preload', 'ezcache' ),
			'enable_preload_desc'    => __( 'Activate the cache preloader. Pages will be crawled in the background to keep the cache warm.', 'ezcache' ),
			'preload_after_clear'    => __( 'Preload after cache clear', 'ezcache' ),
			'crawl_homepage'         => __( 'Crawl homepage links', 'ezcache' ),
			'sitemap_url'            => __( 'Sitemap URL (optional)', 'ezcache' ),
			'sitemap_url_desc'       => __( 'Leave blank to auto-detect /wp-sitemap.xml, /sitemap_index.xml or /sitemap.xml.', 'ezcache' ),
			'urls_per_batch'         => __( 'URLs per batch', 'ezcache' ),
			'frontend_optimizations' => __( 'Front-end Optimizations', 'ezcache' ),
			'lazy_load_images'       => __( 'Lazy load images', 'ezcache' ),
			'lazy_load_iframes'      => __( 'Lazy load iframes', 'ezcache' ),
			'defer_js'               => __( 'Defer JavaScript', 'ezcache' ),
			'defer_js_desc'          => __( 'Adds defer to local script tags to reduce render-blocking JS.', 'ezcache' ),
			'defer_js_exclusions'    => __( 'Defer JS exclusions', 'ezcache' ),
			'defer_js_exclusions_desc' => __( 'One match per line. URLs containing any of these strings will not be deferred.', 'ezcache' ),
			'remove_query_strings'   => __( 'Remove query strings from static assets', 'ezcache' ),
			'dns_prefetch'           => __( 'DNS prefetch hosts', 'ezcache' ),
			'preconnect'             => __( 'Preconnect hosts', 'ezcache' ),
			'heartbeat_control'      => __( 'Heartbeat Control', 'ezcache' ),
			'enable_heartbeat'       => __( 'Enable heartbeat control', 'ezcache' ),
			'mode'                   => __( 'Mode', 'ezcache' ),
			'reduce_activity'        => __( 'Reduce activity (60s ticks)', 'ezcache' ),
			'disable_completely'     => __( 'Disable completely', 'ezcache' ),
			'cdn'                    => __( 'CDN', 'ezcache' ),
			'enable_cdn'             => __( 'Enable CDN rewriting', 'ezcache' ),
			'cdn_url'                => __( 'CDN URL', 'ezcache' ),
			'cdn_url_desc'           => __( 'Static assets under /wp-content/uploads, /themes and /plugins will be rewritten to this host.', 'ezcache' ),
			'database_cleanup'       => __( 'Database Cleanup', 'ezcache' ),
			'delete_revisions'       => __( 'Delete post revisions', 'ezcache' ),
			'delete_auto_drafts'     => __( 'Delete auto drafts', 'ezcache' ),
			'delete_trashed_posts'   => __( 'Delete trashed posts', 'ezcache' ),
			'delete_spam_comments'   => __( 'Delete spam comments', 'ezcache' ),
			'delete_trashed_comments' => __( 'Delete trashed comments', 'ezcache' ),
			'delete_transients'      => __( 'Delete expired transients', 'ezcache' ),
			'delete_orphan_meta'     => __( 'Delete orphan post meta', 'ezcache' ),
			'optimize_tables'        => __( 'Run OPTIMIZE TABLE on all tables', 'ezcache' ),
			'cleanup_schedule'       => __( 'Automatic cleanup schedule', 'ezcache' ),
			'never'                  => __( 'Never (manual only)', 'ezcache' ),
			'run_cleanup_now'        => __( 'Run Cleanup Now', 'ezcache' ),
			'preload_started'        => __( 'Preload started in the background.', 'ezcache' ),
			'preload_stopped'        => __( 'Preload cancelled.', 'ezcache' ),
			'db_cleaned'             => __( 'Database cleanup completed.', 'ezcache' ),
			'saving'                 => __( 'Saving', 'ezcache' ),
			'error'                  => __( 'An error occurred', 'ezcache' ),
			...
			'ignore_query_params'                       => __( 'Ignore tracking parameters when caching', 'ezcache' ),
			'ignore_query_params_description'           => __( 'Advanced: serve the same cached page regardless of the value of tracking parameters (such as gclid or utm_source). Useful for ad-campaign traffic so every click is served from cache instead of creating a new cache entry per click.', 'ezcache' ),
			'ignored_query_params_list_label'           => __( 'Parameters to ignore (one per line; add a trailing * to match by prefix, e.g. utm_*)', 'ezcache' ),
			...
			'combine_head_inline_js'                    => __( 'Combine Inline JavaScript in Head', 'ezcache' ),
			'combine_head_inline_js_description'        => __( 'Combine JavaScript scripts found in the head section of the HTML into the combined JS file.', 'ezcache' ),
			'combine_body_inline_js'                    => __( 'Combine Inline JavaScript in Body', 'ezcache' ),
			'combine_body_inline_js_description'        => __( 'Combine JavaScript scripts found in the body section of the HTML into the combined JS file.', 'ezcache' ),
			'combine_js_elementor_notice'               => __( 'Elementor generates a unique JS file for each post/page which can cause the cache to use large amounts of disk space, to fix this make sure to exclude Elementor JS files (`elementor/js/post-*`) from optimizations.', 'ezcache' ),
			...
			'combine_css_elementor_notice'              => __( 'Elementor generates a unique CSS file for each post/page which can cause the cache to use large amounts of disk space, to fix this make sure to exclude Elementor CSS files (`elementor/css/post-*`) from optimizations.', 'ezcache' ),
			...
			'critical_css_description'                       => __( "Critical CSS is the minimum set of blocking CSS required to render the first screen's worth of content to the user in order to render content to the user as fast as possible.\nThe Critical CSS can (and should) be manually created to fit the website perfectly, however there are tools that can automatically generate the Critical CSS for you.", 'ezcache' ),
			'critical_css_more_information'                  => __( 'More information about Critical CSS', 'ezcache' ),
			'critical_css_online_tool'                       => __( 'Online Critical CSS Generator', 'ezcache' ),
